...
document.addEventListener("DOMContentLoaded", () => {
  // Immediate sliding animation for the top image container on page load
  const topSlidingColor = document.querySelector(".top-img-container .sliding-color");
  if (topSlidingColor) {
    topSlidingColor.classList.add("slide-down");
  }

  // Selecting all relevant components
  const components = document.querySelectorAll(".top-work-card, .bottom-container");
    ...
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          // Trigger sliding color animation for the bottom-container elements
          if (entry.target.classList.contains("bottom-container")) {
  ...
  // Observe each component (both top-work-card and bottom-container)
